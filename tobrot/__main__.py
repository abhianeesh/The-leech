...
if __name__ == "__main__" :
    ...
    status_message_handler = MessageHandler(
        status_message_f,
        filters=Filters.command(["status@TheOthetRoBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(status_message_handler)
    #
    cancel_message_handler = MessageHandler(
        cancel_message_f,
        filters=Filters.command(["cancel@TheOthetRoBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command(["help@TheOthetRoBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command(["savethumbnail@TheOthetRoBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command(["clearthumbnail@TheOthetRoBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
